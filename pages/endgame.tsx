    ...
    <div className="flex flex-col min-h-screen justify-between items-center">
      ...
      <div className="flex flex-col w-screen justify-center items-center">
        ...
        <div className='flex flex-col justify-center text-center mt-5 items-center'>
          <div className="mx-5 bg-slate-500 text-white rounded-md w-36 px-1 py-2 text-l md:text-xl">Your Score: {lastScore}</div>
          <div className="mx-5 bg-slate-500 text-white rounded-md w-28 mt-2 px-3 py-1.5 text-xs md:text-sm">High Score: {highScore}</div>
        </div>
        {(lastTweetType == "ai") && <>
          <h3 className="my-2 mx-10 w-1/2 md:w-1/3 text-base md:text-lg text-center">
            This Tweet was actually <wbr/><b>generated by AI</b>.<br/> Imagine if the entire internet was like this! If only there was a way to <Link className="no-underline hover:underline" href="https://worldcoin.org">prove you're human...</Link>
          </h3>
        </>}
        {(lastTweetType == "human") && <>
          <h3 className="my-2 mx-10 w-1/3 text-base md:text-lg text-center">
            This is actually <wbr/><b>a real Tweet</b>!<br/> It's good to be skeptical, but an internet where we can't trust what we see isn't a very useful internet at all.
          </h3>
        </>}
        {lastTweet && <>
